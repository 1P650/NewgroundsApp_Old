...
public  class SlidingPanel extends LinearLayout {
...

    {
        ...
        stateControlArrow.setImageDrawable(ContextCompat.getDrawable(getContext(), R.drawable.arrow_icon));
    ...
    public void expand() {
        isExpanded = true;
        ((Animatable) stateControlArrow.getDrawable()).start();
        expandSelf(this, 300);
    }
    ...
    private void collapseSelf(final SlidingPanel widget, int duration){
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                widget.getLayoutParams().width = (int) animation.getAnimatedValue();
                widget.requestLayout();
                if((int) animation.getAnimatedValue() == PANEL_MIN_WIDTH) {widget.setVisibility(GONE);}
            }
        });
        valueAnimator.setInterpolator(new DecelerateInterpolator());
        valueAnimator.setDuration(duration);
    ...
    public void setMenu(Menu menu){
        ...
        for (int i = menu.size()-1; i>=0; i--){
            ...
            panel[i].setTag(menu.getItem(i).getTitle());
